        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col">
                    ...
                    <div class="card">
                        <form action="/expenses/{{ $expense->id }}" method="POST" autocomplete="off" spellcheck="false">
                            @csrf
                            @method('put')
                            <div class="card-body">
                                    ...
                                    <div class="col-md-5">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control @error('name') is-invalid @enderror" required
                                                id="name" name="name" placeholder="Masukkan nama pengeluaran..." value="{{ old('name', $expense->name) }}">
                                            @error('name')
                                    ...
                                    <div class="col-md-5">
                                        <div class="form-group">
                                            ...
                                            <div class="input-group mb-3">
                                                ...
                                                <input id="amount" type="text" name="amount" class="form-control rupiah @error('amount') is-invalid @enderror" placeholder="0,00" value="{{ old('amount', $expense->amount) ? change_decimal_format_to_currency(old('amount', $expense->amount)) : '' }}" required>
                                                @error('amount')
                                                    <span class="invalid-feedback">{{ $message }}</span>
                                                @enderror
                                            </div>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            // Initial date
            let start = moment();
            @if(old('date', $expense->date))
                startStr = "{{ old('date', $expense->date) }}";
